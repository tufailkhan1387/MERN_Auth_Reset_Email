...
const { user, restaurant, order, orderCharge, orderStatus, orderHistory, address, charge, vehicleType } = require ("../models");
const { Op, where } = require("sequelize");
const { orderDetails } = require('../controllers/user');
require('dotenv').config();

module.exports = async function getFare(pickUpId, dropOffId, vehicleTypeId, orderId){
  const orderdata = await order.findOne({
    where: {id: orderId },
    attributes: [ 'id', 'orderNum', 'total', 'driverId', 'pickUpId', 'dropOffId', 'vehicleTypeId', 'userId', 'orderApplicationId' ],
    include: [
        { model:orderCharge, attributes: [ 'driverEarnings' ] },
        { model:restaurant, attributes: [ 'lat', 'lng', 'businessName', 'countryCode', 'phoneNum', 'address', 'city', 'zipCode' ], include: { model:user } },
        { model:user, as : 'customerId', attributes: [ 'id', 'firstName', 'lastName', 'countryCode', 'phoneNum', 'deviceToken' ] },
    ]
});
...
resp = await axios.get(`${process.env.MAPS_URL}&origin=${pickupLat},${pickupLng}&destination=${dropOffAddress.lat},${dropOffAddress.lng}&key=AIzaSyDoVmHrVkO68EObrVfhWrzgbAHHPQ9McMM`);
...
const order_accepted_on  = await orderHistory.findOne({where: {orderId, orderStatusId:2}});
const order_end_on  = await orderHistory.findOne({where: {orderId, orderStatusId:10}, order: [ ['id', 'DESC'] ]});
// let eTime = resp.data.routes[0].legs[0].duration.value / 60;
let eTime = (order_end_on.createdAt - order_accepted_on.createdAt) / 1000 / 60;
